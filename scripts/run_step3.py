#!/usr/bin/env python
"""Step 3: SchNet + EGGROLL + GP (Joint Co-Evolution)."""
...
from src.models.schnet import SchNet, build_schnet_model
from src.trainers.step3_trainer import Step3Trainer
from src.utils.utils import seed_everything


def run_step3(config: dict, device: torch.device):
    dataset_name = config['dataset']['name']

    # __ Seed everything FIRST ________________________________________
    train_seed = config['random_seed_train']
    seed_everything(train_seed)
    print(f"random_seed_train={train_seed}")

    print(f"\n{'='*60}")
    print(f"Step 3: SchNet + EGGROLL + GP _ {dataset_name.upper()}")
    print(f"{'='*60}")

    # __ Load or prepare data _________________________________________
    ...
    # __ Create dataloaders ___________________________________________
    # NOTE: conformers are now sorted by energy in data pipeline
    train_loader, valid_loader, test_loader = create_dataloaders(
        config, train_df, valid_df, test_df
    )

    # __ Build model __________________________________________________
    model = build_schnet_model(config)
    print(f"Model: {model.num_params:,} params, {model.num_trainable_params:,} trainable")

    # __ Load Step 2 pretrained weights _______________________________
    step2_path = config.get('step3', {}).get('pretrained_path', None)
    if step2_path and os.path.exists(step2_path):
        print(f"\nLoading Step 2 weights from: {step2_path}")
        state_dict = torch.load(step2_path, map_location=device, weights_only=True)
        model.load_state_dict(state_dict)
        print(f"  Loaded successfully")
    else:
        print(f"\nWARNING: No Step 2 pretrained weights found at: {step2_path}")
        print(f"  Training from scratch (not recommended)")

    # __ Optional: verify loaded weights ______________________________
    ...
    # __ Train ________________________________________________________
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    exp_dir = os.path.join(
        config['experiment']['output_dir'],
        f"step3/{dataset_name}/seed_{split_seed}/{timestamp}",
    )
...
def main(cfg: DictConfig):
    ...
    results = run_step3(config, device)
    ...
    print(f"\nBest GP formula: {results.get('best_gp_tree', 'N/A')}")
